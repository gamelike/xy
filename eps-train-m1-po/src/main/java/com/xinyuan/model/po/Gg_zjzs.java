 ...
 * 表名:GG_ZJZS
 */
public class Gg_zjzs extends BasePo<Gg_zjzs> {
    // 序列化版本号
    private static final long serialVersionUID = 1L;

    public static final RowMapper<Gg_zjzs> ROW_MAPPER = new BeanPropertyRowMapper<>(Gg_zjzs.class);
    ...
    public SqlAndParameters<Map<String, Object>> getSelectSql_(String where, Map<String, Object> parameters) {
        return new SqlAndParameters<>("select id, danhwb, duohwb, xlk, xlkdx, dxk, fxk, rq, rqsj, yh, yhmc, jg, jgmc, qy, zt, tp, cjsj, beiz, sxh from " + this.getTableName_() + " " + where, parameters);
    }
     ...
     */
    @Override
    public Gg_zjzs mapRow(ResultSet rs, int i) throws SQLException {
     ...
     */
    @Override
    public Gg_zjzs clone() {
        try {
            Gg_zjzs gg_zjzs = (Gg_zjzs) super.clone();
            //主键
            if (this.isset_id) {
                gg_zjzs.setId(this.getId());
            }
            //普通属性
            if (this.isset_danhwb) {
                gg_zjzs.setDanhwb(this.getDanhwb());
            }
            if (this.isset_duohwb) {
                gg_zjzs.setDuohwb(this.getDuohwb());
            }
            if (this.isset_xlk) {
                gg_zjzs.setXlk(this.getXlk());
            }
            if (this.isset_xlkdx) {
                gg_zjzs.setXlkdx(this.getXlkdx());
            }
            if (this.isset_dxk) {
                gg_zjzs.setDxk(this.getDxk());
            }
            if (this.isset_fxk) {
                gg_zjzs.setFxk(this.getFxk());
            }
            if (this.isset_rq) {
                gg_zjzs.setRq(this.getRq());
            }
            if (this.isset_rqsj) {
                gg_zjzs.setRqsj(this.getRqsj());
            }
            if (this.isset_yh) {
                gg_zjzs.setYh(this.getYh());
            }
            if (this.isset_yhmc) {
                gg_zjzs.setYhmc(this.getYhmc());
            }
            if (this.isset_jg) {
                gg_zjzs.setJg(this.getJg());
            }
            if (this.isset_jgmc) {
                gg_zjzs.setJgmc(this.getJgmc());
            }
            if (this.isset_qy) {
                gg_zjzs.setQy(this.getQy());
            }
            if (this.isset_zt) {
                gg_zjzs.setZt(this.getZt());
            }
            if (this.isset_tp) {
                gg_zjzs.setTp(this.getTp());
            }
            if (this.isset_cjsj) {
                gg_zjzs.setCjsj(this.getCjsj());
            }
            if (this.isset_beiz) {
                gg_zjzs.setBeiz(this.getBeiz());
            }
            if (this.isset_sxh) {
                gg_zjzs.setSxh(this.getSxh());
            }
            return gg_zjzs;
        } catch (Exception e) {
