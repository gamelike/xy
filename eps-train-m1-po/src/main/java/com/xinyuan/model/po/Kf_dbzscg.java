 ...
 * 表名:kf_dbzscg
 */
public class Kf_dbzscg extends BasePo<Kf_dbzscg> {
    // 序列化版本号
    private static final long serialVersionUID = 1L;

    public static final RowMapper<Kf_dbzscg> ROW_MAPPER = new BeanPropertyRowMapper<>(Kf_dbzscg.class);
    ...
    */
     ...
     */
    @Override
    public Kf_dbzscg mapRow(ResultSet rs, int i) throws SQLException {
     ...
     */
    @Override
    public Kf_dbzscg clone() {
        try {
            Kf_dbzscg kf_dbzscg = (Kf_dbzscg) super.clone();
            //主键
            if (this.isset_id) {
                kf_dbzscg.setId(this.getId());
            }
            //普通属性
            if (this.isset_danhwb) {
                kf_dbzscg.setDanhwb(this.getDanhwb());
            }
            if (this.isset_duohwb) {
                kf_dbzscg.setDuohwb(this.getDuohwb());
            }
            if (this.isset_xlk) {
                kf_dbzscg.setXlk(this.getXlk());
            }
            if (this.isset_xlkdx) {
                kf_dbzscg.setXlkdx(this.getXlkdx());
            }
            if (this.isset_dxk) {
                kf_dbzscg.setDxk(this.getDxk());
            }
            if (this.isset_fxk) {
                kf_dbzscg.setFxk(this.getFxk());
            }
            if (this.isset_rq) {
                kf_dbzscg.setRq(this.getRq());
            }
            if (this.isset_rqsj) {
                kf_dbzscg.setRqsj(this.getRqsj());
            }
            if (this.isset_yh) {
                kf_dbzscg.setYh(this.getYh());
            }
            if (this.isset_yhmc) {
                kf_dbzscg.setYhmc(this.getYhmc());
            }
            if (this.isset_jg) {
                kf_dbzscg.setJg(this.getJg());
            }
            if (this.isset_jgmc) {
                kf_dbzscg.setJgmc(this.getJgmc());
            }
            if (this.isset_qy) {
                kf_dbzscg.setQy(this.getQy());
            }
            if (this.isset_qydx) {
                kf_dbzscg.setQydx(this.getQydx());
            }
            if (this.isset_tp) {
                kf_dbzscg.setTp(this.getTp());
            }
            if (this.isset_fj) {
                kf_dbzscg.setFj(this.getFj());
            }
            if (this.isset_cjsj) {
                kf_dbzscg.setCjsj(this.getCjsj());
            }
            if (this.isset_beiz) {
                kf_dbzscg.setBeiz(this.getBeiz());
            }
            if (this.isset_sxh) {
                kf_dbzscg.setSxh(this.getSxh());
            }
            return kf_dbzscg;
        } catch (Exception e) {
