 ...
 * 表名:kf_tree
 */
public class Kf_tree extends BasePo<Kf_tree> {
    // 序列化版本号
    private static final long serialVersionUID = 1L;

    public static final RowMapper<Kf_tree> ROW_MAPPER = new BeanPropertyRowMapper<>(Kf_tree.class);
    ...
    */
     ...
     */
    @Override
    public Kf_tree mapRow(ResultSet rs, int i) throws SQLException {
     ...
     */
    @Override
    public Kf_tree clone() {
        try {
            Kf_tree kf_tree = (Kf_tree) super.clone();
            //主键
            if (this.isset_id) {
                kf_tree.setId(this.getId());
            }
            //普通属性
            if (this.isset_pid) {
                kf_tree.setPid(this.getPid());
            }
            if (this.isset_bs) {
                kf_tree.setBs(this.getBs());
            }
            if (this.isset_bt) {
                kf_tree.setBt(this.getBt());
            }
            if (this.isset_lj) {
                kf_tree.setLj(this.getLj());
            }
            if (this.isset_sxh) {
                kf_tree.setSxh(this.getSxh());
            }
            return kf_tree;
        } catch (Exception e) {
