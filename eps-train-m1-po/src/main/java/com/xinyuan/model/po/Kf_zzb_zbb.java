 ...
 * 表名:kf_zzb_zbb
 */
public class Kf_zzb_zbb extends BasePo<Kf_zzb_zbb> {
    // 序列化版本号
    private static final long serialVersionUID = 1L;

    public static final RowMapper<Kf_zzb_zbb> ROW_MAPPER = new BeanPropertyRowMapper<>(Kf_zzb_zbb.class);
    ...
    */
    ...
    public SqlAndParameters<Map<String, Object>> getSelectSql_(String where, Map<String, Object> parameters) {
        return new SqlAndParameters<>("select id, fid, danhwb, duohwb, xlk, xlkdx, dxk, fxk, rq, rqsj, yh, yhmc, jg, jgmc, qy, qydx, tp, fj, cjsj, beiz, sxh from " + this.getTableName_() + " " + where, parameters);
    }
     ...
     */
    @Override
    public Kf_zzb_zbb mapRow(ResultSet rs, int i) throws SQLException {
     ...
     */
    @Override
    public Kf_zzb_zbb clone() {
        try {
            Kf_zzb_zbb kf_zzb_zbb = (Kf_zzb_zbb) super.clone();
            //主键
            if (this.isset_id) {
                kf_zzb_zbb.setId(this.getId());
            }
            //普通属性
            if (this.isset_fid) {
                kf_zzb_zbb.setFid(this.getFid());
            }
            if (this.isset_danhwb) {
                kf_zzb_zbb.setDanhwb(this.getDanhwb());
            }
            if (this.isset_duohwb) {
                kf_zzb_zbb.setDuohwb(this.getDuohwb());
            }
            if (this.isset_xlk) {
                kf_zzb_zbb.setXlk(this.getXlk());
            }
            if (this.isset_xlkdx) {
                kf_zzb_zbb.setXlkdx(this.getXlkdx());
            }
            if (this.isset_dxk) {
                kf_zzb_zbb.setDxk(this.getDxk());
            }
            if (this.isset_fxk) {
                kf_zzb_zbb.setFxk(this.getFxk());
            }
            if (this.isset_rq) {
                kf_zzb_zbb.setRq(this.getRq());
            }
            if (this.isset_rqsj) {
                kf_zzb_zbb.setRqsj(this.getRqsj());
            }
            if (this.isset_yh) {
                kf_zzb_zbb.setYh(this.getYh());
            }
            if (this.isset_yhmc) {
                kf_zzb_zbb.setYhmc(this.getYhmc());
            }
            if (this.isset_jg) {
                kf_zzb_zbb.setJg(this.getJg());
            }
            if (this.isset_jgmc) {
                kf_zzb_zbb.setJgmc(this.getJgmc());
            }
            if (this.isset_qy) {
                kf_zzb_zbb.setQy(this.getQy());
            }
            if (this.isset_qydx) {
                kf_zzb_zbb.setQydx(this.getQydx());
            }
            if (this.isset_tp) {
                kf_zzb_zbb.setTp(this.getTp());
            }
            if (this.isset_fj) {
                kf_zzb_zbb.setFj(this.getFj());
            }
            if (this.isset_cjsj) {
                kf_zzb_zbb.setCjsj(this.getCjsj());
            }
            if (this.isset_beiz) {
                kf_zzb_zbb.setBeiz(this.getBeiz());
            }
            if (this.isset_sxh) {
                kf_zzb_zbb.setSxh(this.getSxh());
            }
            return kf_zzb_zbb;
        } catch (Exception e) {
