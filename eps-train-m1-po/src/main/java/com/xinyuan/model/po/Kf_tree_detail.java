 ...
 * 表名:kf_tree_detail
 */
public class Kf_tree_detail extends BasePo<Kf_tree_detail> {
    // 序列化版本号
    private static final long serialVersionUID = 1L;

    public static final RowMapper<Kf_tree_detail> ROW_MAPPER = new BeanPropertyRowMapper<>(Kf_tree_detail.class);
    ...
    * 默认构造函数
    */
    public Kf_tree_detail() {
    }

    /**
    * 根据主键构造对象
    */
    public Kf_tree_detail(String id) {
        this.setId(id);
    }
    ...
    public SqlAndParameters<Map<String, Object>> getSelectSql_(String where, Map<String, Object> parameters) {
        return new SqlAndParameters<>("select id, treeid, danhwb, duohwb, xlk, xlkdx, dxk, fxk, rq, rqsj, yh, yhmc, jg, jgmc, qy, qydx, tp, fj, cjsj, beiz, sxh from " + this.getTableName_() + " " + where, parameters);
    }
     ...
     */
    @Override
    public Kf_tree_detail mapRow(ResultSet rs, int i) throws SQLException {
     ...
     */
    @Override
    public Kf_tree_detail clone() {
        try {
            Kf_tree_detail kf_tree_detail = (Kf_tree_detail) super.clone();
            //主键
            if (this.isset_id) {
                kf_tree_detail.setId(this.getId());
            }
            //普通属性
            if (this.isset_treeid) {
                kf_tree_detail.setTreeid(this.getTreeid());
            }
            if (this.isset_danhwb) {
                kf_tree_detail.setDanhwb(this.getDanhwb());
            }
            if (this.isset_duohwb) {
                kf_tree_detail.setDuohwb(this.getDuohwb());
            }
            if (this.isset_xlk) {
                kf_tree_detail.setXlk(this.getXlk());
            }
            if (this.isset_xlkdx) {
                kf_tree_detail.setXlkdx(this.getXlkdx());
            }
            if (this.isset_dxk) {
                kf_tree_detail.setDxk(this.getDxk());
            }
            if (this.isset_fxk) {
                kf_tree_detail.setFxk(this.getFxk());
            }
            if (this.isset_rq) {
                kf_tree_detail.setRq(this.getRq());
            }
            if (this.isset_rqsj) {
                kf_tree_detail.setRqsj(this.getRqsj());
            }
            if (this.isset_yh) {
                kf_tree_detail.setYh(this.getYh());
            }
            if (this.isset_yhmc) {
                kf_tree_detail.setYhmc(this.getYhmc());
            }
            if (this.isset_jg) {
                kf_tree_detail.setJg(this.getJg());
            }
            if (this.isset_jgmc) {
                kf_tree_detail.setJgmc(this.getJgmc());
            }
            if (this.isset_qy) {
                kf_tree_detail.setQy(this.getQy());
            }
            if (this.isset_qydx) {
                kf_tree_detail.setQydx(this.getQydx());
            }
            if (this.isset_tp) {
                kf_tree_detail.setTp(this.getTp());
            }
            if (this.isset_fj) {
                kf_tree_detail.setFj(this.getFj());
            }
            if (this.isset_cjsj) {
                kf_tree_detail.setCjsj(this.getCjsj());
            }
            if (this.isset_beiz) {
                kf_tree_detail.setBeiz(this.getBeiz());
            }
            if (this.isset_sxh) {
                kf_tree_detail.setSxh(this.getSxh());
            }
            return kf_tree_detail;
        } catch (Exception e) {
